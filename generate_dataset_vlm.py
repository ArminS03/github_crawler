...
def batch_check_scientific_figures_gemini(image_paths):
    prompt = (
        f"You will be given {len(image_paths)} images. Determine which of these images are Scientific figures and then return their indices. The indices you return should start from 0. (The first image has index 0 and the last image index {len(image_paths)-1})\n"
        "Is it a scientific figure, if it was entirely created by any of these libraries: matplotlib, seaborn, plotly, bokeh, altair, ggplot.\n"
...
def main():
    ...
    for root, dirs, files in os.walk(GATHER_DATA):
        for project in tqdm(dirs):
            project_path = os.path.join(root, project)
            images = get_image_files(project_path)
            py_files = find_python_files(project_path)
            if not images or not py_files:
                continue
            ...
            for img_path in confirmed_figures:
                # Concatenate all Python codes in the folder

                # Generate code to recreate the figure using Gemini
                with open(img_path, "rb") as img_file:
                    image_bytes = img_file.read()
                prompt = (
                    "Given this scientific figure and the Python code below, "
                    "determine if any of the provided code likely produce this figure? (Some differences in the label and colors is not important)\n"
                    "If yes, generate Python code (using matplotlib, seaborn, plotly, bokeh, altair, or ggplot)."
                    "You should try to mimic the style of the provided code and image as closely as possible. (save the figure to disk as 'output.png'). "
                    "If not, just say state the reason in the code section instead.\n\n"
                    "PYTHON CODE:\n" + all_codes
                )
                ...
                try:
                    ...
                    print(f"[ERROR] Code execution timed out for {img_path}")
                except subprocess.CalledProcessError as e:
                    print(f"[ERROR] Code execution failed for {img_path}: {e}")
